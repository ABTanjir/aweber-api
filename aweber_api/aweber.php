...
class AWeberAPI extends AWeberAPIBase {
     ...
     * @return null
     */
    public function __construct($key, $secret, $appId) {
        // Load key / secret
    ...
    public function setEmail($email, $name, $listId) {
        $param = array(
            'name'=>$name, 
            'email'=>$email
        );        
        $body = $this->adapter->request('POST', "/accounts/$this->appId/lists/$listId/subscribers", $param);
        $accounts = $this->readResponse($body, "/accounts/$this->appId/lists/$listId/subscribers");
        return $accounts;
    }

    public function findEmail($email, $listid= false){
        if($listid != false){
            // $param = array(
            //     'ws.op'=>'find',
            //     'email'=>$email
            // );
            // $body = $this->adapter->request('GET', "/accounts/$this->appId/lists/$listid", $param);
            // $accounts = $this->readResponse($body, "/accounts/$this->appId/lists/$listid", $param);
        }else{
            $param = array(
                'ws.op'=>'findSubscribers',
                'email'=>$email
            );
            $body = $this->adapter->request('GET', "/accounts/$this->appId", $param);
            $accounts = $this->readResponse($body, "/accounts/$this->appId", $param); 
        }
